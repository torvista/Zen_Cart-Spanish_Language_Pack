<?php //Spanish Language Pack UTF-8: http://zencartspanish.svn.sourceforge.net/viewvc/zencartspanish/
/**
 * @package languageDefines
 * @copyright Copyright 2003-2009 Zen Cart Development Team
 * @copyright Portions Copyright 2003 osCommerce
 * @license http://www.zen-cart.com/license/2_0.txt GNU Public License V2.0
 * @version $Id: checkout_success.php 14198 2009-08-18 22:32:11Z drbyte $
 */

define('NAVBAR_TITLE_1', 'Pago');
define('NAVBAR_TITLE_2', 'Listo - Gracias');

define('HEADING_TITLE', '¡Muchas gracias por su confianza!');

define('TEXT_SUCCESS', '');
define('TEXT_NOTIFY_PRODUCTS', 'Avísenme cuando haya cambios en los siguientes artículos:');
define('TEXT_SEE_ORDERS', 'Puede ver su historial de pedidos y descargar las facturas yendo a <a href="' . zen_href_link(FILENAME_ACCOUNT, '', 'SSL') . '">Mi Cuenta</a> y pulsando en "Ver todos los pedidos".');
define('TEXT_CONTACT_STORE_OWNER', 'Por favor, dirija cualquier pregunta que tenga a <a href="' . zen_href_link(FILENAME_CONTACT_US) . '">atención al cliente</a>.');
define('TEXT_THANKS_FOR_SHOPPING', '¡Muchas gracias por su confianza!');

define('TABLE_HEADING_COMMENTS', '');

define('FOOTER_DOWNLOAD', 'También puede descargar sus productos más tarde en \'%s\'');

define('TEXT_YOUR_ORDER_NUMBER', '<strong>Su número de pedido es:</strong> ');

define('TEXT_CHECKOUT_LOGOFF_GUEST', 'NOTA: Para procesar su pedido, se ha creado una cuenta temporal. Se puede cerrar esa cuenta haciendo click en "Log Off". Adicionalmente "Salir" asegura que la información sobre su factura y pedido no será visible para la próxima persona que utilice este ordenador. Si quieres seguir comprando ¡adelante! Se puede salir en cualquier momento a través del enlace al principio de la página.');
define('TEXT_CHECKOUT_LOGOFF_CUSTOMER', '¡Muchas gracias por su confianza! Haga un click en "Salir" para asegurarse  que la información sobre su factura y pedido no será visible para la próxima persona que utilice este ordenador.');